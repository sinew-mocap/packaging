<#
  Build + sign the Sinew MSIX from already-built Win64 executables.
  Requires Windows + Windows SDK (makeappx.exe, signtool.exe).

  -BinDir   folder with sinew_tui.exe, anny_demo.exe, hmd_reader.exe, vr_devices.exe (+ runtime DLLs/assets)
  -Version  4-part version, e.g. 0.4.1.0
  ...
  ex: pwsh packaging/msix/pack.ps1 -BinDir stage-win/bin -Version 0.4.1.0
#>
[CmdletBinding()]
param(
...
$root = Join-Path ([System.IO.Path]::GetTempPath()) ("sinew-msix-" + [guid]::NewGuid())
...
$msix = Join-Path $OutDir "v-sekai-sinew-$Version.msix"
...
if (-not $PfxPath) {
  ...
  $PfxPath = Join-Path $OutDir "sinew-test.pfx"; $PfxPassword = "test"
